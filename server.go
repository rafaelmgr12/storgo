...
func (s *FileServer) Get(key string) (io.Reader, error) {
	...
	for _, peer := range s.peers {
		// Firs read the file size so we can limit the amount of bytes that we read
		// from the connection, so it will not keeping hanging.
		var fileSize int64
		...
		fmt.Printf("[%s] received (%d) bytes over the newtork from (%s)", s.Transport.Addr(), n, peer.RemoteAddr())
		peer.CloseStream()
...
func (s *FileServer) handleMessageGetFile(from string, msg MessageGetFile) error {
	...
	// First send the "IncomingStream" byte to the peer and the we can send
